...
installable throughout the transition (a mixed store).

### MVP: phased plan

Ed25519 keys, DSSE envelopes, key held by the security-proxy, a committed
keyring — no Fulcio/Rekor/TUF/OPA. ~2 weeks. Each phase is independently testable
and lands behind a flag, so nothing changes for existing (unsigned) stores until
enforcement is switched on.

- **Phase S0 — Verify primitives + keyring (pure, no infra).** New
...
- **Phase S1 — security-proxy `sign` route.** Add an Ed25519 `sign` route in
...
- **Phase S2 — Sign on upload.** In `REAPIRemoteSync._upload_tarball` (once
...
- **Phase S3 — Verify on consume.** Hook verification into `install.py`
...
- **Phase S4 — Keyring distribution + rollout.** Ship the keyring (in alibuild or
  alidist, or a known URL), itself signed by a root key so it can't be tampered
  with (TUF-lite: verify-keyring-before-use); document bootstrap. Revocation MVP =
  a `revoked` list in the keyring. Flip the default off → warn → `--require-signature`
  once producers sign; unsigned legacy entries stay installable in warn mode.
...
snapshots, validate-system actions, and legacy-store `migrate` are implemented.
**Signing (above) and Sapling source snapshots are the main outstanding items.**
