...
_aliBuild_complete() {
  ...
  case "$subcmd" in
    build)
      case "$prev" in
        -a|--architecture|-z|--devel-prefix|-e|-j|--jobs|--plugin|--docker-image|--docker-extra-args|-v|--remote-store|--write-store|--ac-store|--sign-url|--sign-token|--signer)
          return ;;
        --storage)
      ...
      if [[ "$cur" == -* ]]; then
        COMPREPLY=( $(compgen -W "
          ...
          --no-snapshot-sources --sign-url --sign-token --signer --no-sign
