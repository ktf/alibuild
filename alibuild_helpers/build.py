...
def doBuild(args, parser):
  ...
  if args.writeStore.startswith("reapi://") and not getattr(args, "noSign", False):
    dieOnError(not (getattr(args, "signUrl", "") and getattr(args, "signToken", "")),
               "uploading to a reapi:// store signs Action Cache entries by default: "
               "pass --sign-url and --sign-token, or --no-sign to upload unsigned.")
  ...
  # Record validate-system actions for the system / prefer_system packages, so the
  # Action Cache holds the full recipe closure: reconstruct can then materialise
  # their recipes (which dependency resolution needs) and re-validate the system
  # requirement on the target host, with no alidist checkout. Only when we are
  # actually uploading (a writable store); a read-only build -- e.g. reconstruct's
  # isolated rebuild -- writes nothing. Backends without an AC (put_ac_entry) skip.
  if not args.onlyDeps and isinstance(syncHelper, REAPIRemoteSync) and \
     getattr(syncHelper, "writeStore", ""):
    for sysname, sysspec in sorted(systemPackageSpecs.items()):
      recipe_text = sysspec.get("fullRecipe") or sysspec.get("recipe") or ""
      syncHelper.put_ac_entry(
        build_validate_system_entry(sysspec, specs, args.architecture), recipe_text,
        sign=True)
