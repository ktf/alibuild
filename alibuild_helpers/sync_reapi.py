...
class REAPIRemoteSync(Boto3RemoteSync):
  ...
  *action* hash and hardcodes the CERN endpoint, this backend (scheme
  ...
  def upload_symlinks_and_tarball(self, spec) -> None:
    ...
    uploads the legacy symlinks and the per-package link *before* calling
    _upload_tarball, so aborting from in there leaves a link with no store object
    -- a half-state the legacy consistency check (sync.py) then refuses to touch,
    permanently blocking the package it was trying to protect."""
