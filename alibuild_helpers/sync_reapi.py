...
class SignatureChecker:
  ...
  def check_closure(self, entries):
    ...
    for package, level, reason in problems:
      (warning if level == "warn" else error)(
        "Signature check for %s: %s", package, reason)
  ...
  def check_blob(self, path, algo, content_hash):
    """Confirm downloaded bytes hash to the signed digest before they are used."""
    actual = file_digest(path, algo)
    dieOnError(actual != content_hash,
               "Downloaded blob hashes to %s:%s but the (signed) Action Cache "
               "entry expects %s:%s -- refusing to use it." %
               (algo, actual, algo, content_hash))


def default_keyring_path(args):
  """Resolve the keyring to verify against: an explicit --trusted-keys wins,
  otherwise keyring.json in the alidist checkout (args.configDir for build,
  args.alidist for reconstruct). Returns "" when none can be located (e.g. the
  recipe-free `install`, which has no alidist)."""
  explicit = getattr(args, "trustedKeys", "") or ""
  if explicit:
    return explicit
  alidist = getattr(args, "configDir", "") or getattr(args, "alidist", "") or ""
  return os.path.join(alidist, "keyring.json") if alidist else ""
...
def signature_checker(args):
  ...
  with no keyring found -- so unsigned stores keep working out of the box. Policy
  ``require`` with no keyring fails fast."""
  from alibuild_helpers import signing
  policy = getattr(args, "requireSignature", signing.POLICY_WARN)
  if policy == signing.POLICY_OFF:
    return None
  path = default_keyring_path(args)
  if not path or not os.path.exists(path):
    dieOnError(policy == signing.POLICY_REQUIRE,
               "--require-signature=require needs a keyring, but none was found: "
               "pass --trusted-keys or add %s." % (path or "alidist/keyring.json"))
    debug("No signing keyring at %s; skipping signature verification (policy=%s).",
          path or "alidist/keyring.json", policy)
    return None
  try:
    keyring = signing.load_keyring(path)
...
class REAPIRemoteSync(Boto3RemoteSync):
  ...
  *action* hash and hardcodes the CERN endpoint, this backend (scheme
  ...
  def __init__(self, remoteStore, writeStore, architecture, workdir,
               ...
               sign_url="", sign_token="", signer="alibuild") -> None:
  ...
  def _sign_ac_entry(self, ac_entry):
    ...
    dieOnError(not self.sign_token,
               "signing a reapi:// upload needs a gate token: pass --sign-token "
               "(or --no-sign to upload unsigned).")
    signature = signing.sign_via_proxy(ac_entry, self.sign_url, self.sign_token,
                                       self.signer)
  ...
  def _verify_download(self, spec, entry, tar_path, algo="", content_hash=""):
    ...
    if content_hash:
      self.verify_checker.check_blob(tar_path, algo, content_hash)
